...
export async function notifyNewUser(loginId, totalCount) {
  if (!BOT_TOKEN || !CHAT_ID) return;
  if (EXCLUDED_IDS.includes(loginId)) return;

  try {
    const countText = totalCount ? `\n현재 총 회원 수: ${totalCount}명` : '';
    const res = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        chat_id: CHAT_ID,
        text: `🆕 신규 가입: ${loginId}${countText}`,
      }),
    });
    const json = await res.json();
    console.log('[TelegramNotify] 신규 가입 알림 응답:', json);
  } catch (err) {
    console.error('[TelegramNotify] 신규 가입 알림 실패:', err);
  }
}
